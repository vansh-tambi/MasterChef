...
export function RecipeLoading({ onCancel }) {
  ...
  return (
    <Card accent={true} badge="PREPARING // SPEC 01" className="w-full space-y-7 sm:space-y-9 overflow-hidden">
      <motion.div
      ...
      >
        ...
        <div className="flex items-center justify-between gap-4 border-b-2 border-panel-border pb-4 -mx-5 sm:-mx-8 px-5 sm:px-8 bg-elevated/40">
          <div className="flex items-center gap-3">
            <motion.div
              animate={shouldReduceMotion ? undefined : { y: [0, -4, 0], rotate: [0, -2, 2, 0] }}
              transition={{ duration: 1.4, repeat: Infinity, ease: 'easeInOut' }}
              className="w-10 h-10 rounded-sm bg-surface border-2 border-panel-border flex items-center justify-center text-xl shadow-[2px_2px_0px_0px_rgba(26,29,32,0.15)] dark:shadow-[2px_2px_0px_0px_#000]"
            >
              🔥
            </motion.div>
